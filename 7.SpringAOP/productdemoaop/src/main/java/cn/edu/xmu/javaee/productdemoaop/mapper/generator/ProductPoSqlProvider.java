...
public class ProductPoSqlProvider {
    ...
    public String selectByJoinExample(ProductPoExample example) {
        SQL sql = new SQL();
        if (example != null && example.isDistinct()) {
            sql.SELECT_DISTINCT("`p`.`id`");
        } else {
            sql.SELECT("`p`.`id`");
        }
        sql.SELECT("`p`.`shop_id`");
        sql.SELECT("`goods_id`");
        sql.SELECT("`category_id`");
        sql.SELECT("`template_id`");
        sql.SELECT("`sku_sn`");
        ...
        sql.SELECT("`unit`");
        sql.SELECT("`origin_place`");
        sql.SELECT("`p`.`creator_id`");
        sql.SELECT("`p`.`creator_name`");
        sql.SELECT("`p`.`modifier_id`");
        sql.SELECT("`p`.`modifier_name`");
        sql.SELECT("`p`.`gmt_create`");
        sql.SELECT("`p`.`gmt_modified`");
        sql.SELECT("`status`");
        sql.SELECT("`commission_ratio`");
        sql.SELECT("`shop_logistic_id`");
        sql.SELECT("`free_threshold`");
